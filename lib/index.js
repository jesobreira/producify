...
producifylib.regexp = {
    ...
    "css": /<link(?: ?(?:rel|type|media)=['"]?(?:stylesheet|text\/css|all|screen)['"]?)* href=['"]?(?!http|\/\/|ftp)\/?([^'"]+)(?<!\.min\.css)(?:\?.*)?['"](?: ?(?:rel|type|media)=['"]?(?:stylesheet|text\/css|all|screen)['"]?)* ?\/?>/ig,
    "js": /<script(?:.*)src=["'](?!http|\/\/|ftp)\/?([^'">]*)(?<=\.js)(?<!\.min\.js)(?:\?.*)?['"](?:.*)>/ig,
    "md": /<markdown>([\s\S]*)<\/markdown>/igm
}

producifylib.checkForUpdates = function() {
        ...
        recursive(target, [function(file, stats) {
            ...
            function(err, files) {
                ...
                files.forEach(function(file) {
                    ...
                    if (opts.minifyJs) {
                        do {
                            var scripts_regexp = RegExp(producifylib.regexp.js.source, producifylib.regexp.js.flags);
                            var jses = scripts_regexp.exec(newfilecontents);
                            if (jses !== null) {
                                var full_tag = jses[0];
                    ...
                    // markdown
                    do {
                        var markdown_regexp = RegExp(producifylib.regexp.md.source, producifylib.regexp.md.flags);
                        var mds = markdown_regexp.exec(newfilecontents);
                        if (mds !== null) {
                            var full_tag = mds[0];
                            var md_raw = mds[1];

                            console.log("Parsing markdown at " + file);
                            var md_parsed = marked(md_raw);
                            newfilecontents = newfilecontents.replace(full_tag, md_parsed)
                        }
                    } while (mds !== null);
